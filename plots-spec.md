...
## Extended catalog (added in the Phase C expansion)

The original spec above covers the *first wave* of artifacts.  The
interactive playground was later expanded to cover `pav`, `pam` (scrunch
counts), `paz` (zap variants), `vap`, `psrstat`, `psradd`, and `pat`
(`.tim` output).  Those new artifacts follow the same naming schema:

```
plots/<command>/<archive>/<descriptor>.png            ← image
plots/<command>/<archive>/<descriptor>.txt            ← stdout
plots/<command>/<archive>/<descriptor>.annot.json     ← span tooltips
plots/<command>/<archive>/<descriptor>.meta.txt       ← post-op vap summary
plots/<command>/<archive>/<descriptor>.meta.annot.json
```

The full filename list for each new family is in
[`tools/HANDOFF.md`](tools/HANDOFF.md), generated by the seven scripts
under `tools/gen/`.  See `_annot_helpers.py` for the `.annot.json` schema
used by the website's `TextOutput` component.
