...
function FoldingAnim({ w = 460, h = 200 }) {
  ...
  const pts = useMemo(() => {
    ...
    for (let i = 0; i < 240; i++) {
      const phase = i / 240;
      const signal = 0.85 * Math.exp(-Math.pow((phase - 0.42) / 0.04, 2))
...
function RfiMorph({ w = 520, h = 280 }) {
  ...
  const cells = useMemo(() => {
    ...
    for (let y = 0; y < 28; y++) {
      for (let x = 0; x < 36; x++) {
        const inBand = x > 11 && x < 19;
        let v = rng() * 0.03;
        if (inBand) {
          const phaseShape = Math.exp(-Math.pow((x - 15) / 2.5, 2));
          const lowFreqBoost = (y > 14 ? (y - 14) / 14 : 0);
          v = Math.min(1, phaseShape * (0.55 + lowFreqBoost) + rng() * 0.08);
        }
        // RFI rows
        const rfiHigh = y === 4 && rng() > 0.25;
        const rfiLow = y === 19 && x > 24 && rng() > 0.4;
        const rfiNarrow = y === 11 && x > 28 && rng() > 0.5;
        const isRfi = rfiHigh || rfiLow || rfiNarrow;
        const rfiV = rfiHigh ? 0.62 + rng() * 0.25 : rfiLow ? 0.4 + rng() * 0.2 : 0.5 + rng() * 0.2;
        a.push({ x, y, v, isRfi, rfiV });
      }
    }
    return a;
  }, []);
  ...
  return (
    <div>
      <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ display: "block", background: "#000", borderRadius: 4 }}>
        <g transform="translate(30, 10)">
          {cells.map((c, i) => {
            const final = c.isRfi ? (1 - z) * c.rfiV + z * c.v : c.v;
            const [r, g, b] = heatColor(final);
            return <rect key={i} x={c.x * cellW} y={c.y * cellH} width={cellW + 0.5} height={cellH + 0.5} fill={`rgb(${r},${g},${b})`} />;
          })}
        </g>
        <text x={w / 2} y={h - 8} textAnchor="middle" fontSize="11" fill="#bbb" fontFamily="var(--font-body)">pulse phase</text>
        <text x={12} y={h / 2} textAnchor="middle" fontSize="11" fill="#bbb" fontFamily="var(--font-body)" transform={`rotate(-90 12 ${h / 2})`}>frequency</text>
        {/* label */}
        <g transform={`translate(${w - 90}, 18)`}>
          <rect width="78" height="22" rx="11" fill="var(--green)" />
          <text x="39" y="15" textAnchor="middle" fontSize="11" fontFamily="var(--font-mono)" fontWeight="700" fill="var(--paper)">
